...
if [ "$1" == "" ]; then
	...
	do
		...
		if [ "$Exec_Hour" == "00:00:00" ] || [ "$Exec_Hour" == "08:00:00" ] || [ "$Exec_Hour" == "16:00:00" ] || [ "$Exec_Hour" == "16:36:00" ]; then
			...
			if [ "$Shodan_Start" == "Successfully initialized" ]; then
				...
				for Target_Query in $(cat Target_Query.txt); do
					...
					shodan search $Target_Query --fields ip_str country:BR 2> /dev/null | sort -u >> Shodan_IPList.txt
					# Tentar realizar parsing. shodan search $Target_Query parse --fields ip_str,port --separator , | cut -d "," f1 | sort -u > Shodan_IPList.txt
				done
					for IP in $(cat Shodan_IPList.txt); do
						Ports=$(shodan search $IP --fields port country:BR >> Shodan_PortList.txt)
